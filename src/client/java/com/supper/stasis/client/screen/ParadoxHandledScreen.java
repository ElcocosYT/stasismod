...
public class ParadoxHandledScreen extends HandledScreen<ParadoxScreenHandler> {
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        int left = this.x;
        int top = this.y;
        context.drawTexture(RenderPipelines.GUI_TEXTURED, MAINFRAME_TEXTURE, left, top, 0.0f, 0.0f, this.backgroundWidth, this.backgroundHeight, this.backgroundWidth, this.backgroundHeight);
        ...
        context.drawTexture(RenderPipelines.GUI_TEXTURED, BLOCKED_BUTTON_TEXTURE, left + INPUT_X, top + INPUT_Y, 0.0f, 0.0f, INPUT_WIDTH, INPUT_HEIGHT, INPUT_WIDTH, INPUT_HEIGHT);
    ...
    protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
        ...
        context.drawText(this.textRenderer, Text.literal("Lapses"), LAPSES_TEXT_X, LAPSES_TEXT_Y, withOpaqueAlpha(0x70888D), false);
        context.drawText(this.textRenderer, this.playerInventoryTitle, this.playerInventoryTitleX, this.playerInventoryTitleY, withOpaqueAlpha(0xFFFFFF), false);
    ...
    private void refreshWidgets() {
        ...
        this.customInput.setEditableColor(withOpaqueAlpha(0x000000));
        this.customInput.setUneditableColor(withOpaqueAlpha(0x000000));
    ...
    private void drawCenteredText(DrawContext context, Text text, int centerX, int y, int color) {
        ...
        context.drawText(this.textRenderer, text, centerX - width / 2, y, withOpaqueAlpha(color), false);
    }
    ...
    private void drawTexturedButton(DrawContext context, Identifier texture, ButtonWidget button, int width, int height, boolean disabled) {
        context.drawTexture(RenderPipelines.GUI_TEXTURED, texture, button.getX(), button.getY(), 0.0f, 0.0f, width, height, width, height);
    ...
    private void playUiSound(SoundEvent sound, float volume, float pitch) {
        ...
        if (client != null) {
            client.getSoundManager().play(PositionedSoundInstance.ui(sound, volume, pitch));
        }
    }
