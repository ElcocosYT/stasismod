...
public class ParadoxScreenHandler extends ScreenHandler {
    ...
    public ItemStack quickMove(PlayerEntity player, int index) {
        ...
        } else if (stack.isOf(Stasis.REINFORCED_DIAMOND) || stack.isOf(Stasis.REINFORCED_NETHERITE)) {
            if (!insertReinforcedMaterialBalanced(stack)) {
                return ItemStack.EMPTY;
            }
    ...
    private boolean insertReinforcedMaterialBalanced(ItemStack stack) {
        boolean movedAny = false;

        while (!stack.isEmpty()) {
            int targetSlotIndex = findBestUpgradeSlot(stack);
            if (targetSlotIndex < 0) {
                break;
            }
            ...
            ItemStack targetStack = targetSlot.getStack();
            int currentCount = targetStack.isEmpty() ? 0 : targetStack.getCount();
            int maxCount = targetSlot.getMaxItemCount();
            int toMove = Math.min(stack.getCount(), maxCount - currentCount);
            if (toMove <= 0) {
                break;
            }
    ...
    private int findBestUpgradeSlot(ItemStack stack) {
        for (int slotIndex = GRIT_SLOT_COUNT; slotIndex < PANEL_SLOT_COUNT; slotIndex++) {
            Slot upgradeSlot = this.slots.get(slotIndex);
            if (!upgradeSlot.canInsert(stack)) {
                continue;
            }

            ItemStack existing = upgradeSlot.getStack();
            if (!existing.isEmpty() && !ItemStack.canCombine(existing, stack)) {
                continue;
            }

            int currentCount = existing.isEmpty() ? 0 : existing.getCount();
            if (currentCount >= upgradeSlot.getMaxItemCount()) {
                continue;
            }
