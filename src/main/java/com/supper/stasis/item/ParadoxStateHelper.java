...
public final class ParadoxStateHelper {
    ...
    private static void applyVisualComponents(ItemStack stack, ParadoxStateComponent state) {
        switch (state.tier()) {
            case TIER_TWO -> stack.set(DataComponentTypes.CUSTOM_MODEL_DATA, new CustomModelDataComponent(MODEL_ASTRAL));
            case TIER_THREE -> stack.set(DataComponentTypes.CUSTOM_MODEL_DATA, new CustomModelDataComponent(MODEL_VOID));
