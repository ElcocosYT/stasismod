...
public final class StasisCommands {
    ...
    private static boolean hasPermissionLevel(ServerCommandSource source, int level) {
        return source.getPermissions() instanceof LeveledPermissionPredicate leveledPermission
                && leveledPermission.getLevel().isAtLeast(PermissionLevel.fromLevel(level));
    }
}
