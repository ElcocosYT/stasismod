...
public class StasisManager {
            ...
            try {
                for (QueuedHit queuedHit : new ArrayList<>(queuedHits)) {
                    ...
                    if (entity instanceof LivingEntity livingEntity) {
                        ...
                        livingEntity.damage(world, queuedHit.source, queuedHit.amount);
                    }
                }
            } finally {
                queuedHits.clear();
        ...
        if (!queuedNonEntityDamage.isEmpty()) {
            ...
            try {
                for (QueuedNonEntityDamage queuedDamage : new ArrayList<>(queuedNonEntityDamage)) {
                    ...
                    if (entity instanceof LivingEntity livingEntity) {
                        resetDamageCooldown(livingEntity);
                        livingEntity.damage(world, queuedDamage.source, queuedDamage.amount);
                    }
                }
            } finally {
                queuedNonEntityDamage.clear();
    ...
    public synchronized boolean emergencyBreak(MinecraftServer server) {
        ...
        if (soundWorld != null) {
            ...
            if (activatingPlayer != null) {
                ...
                BlockPos spawnPos = soundWorld.getSpawnPoint().getPos();
                x = spawnPos.getX() + 0.5;
                y = spawnPos.getY() + 0.5;
                z = spawnPos.getZ() + 0.5;
            }
    ...
    public synchronized void captureTransitionTickState(Entity entity) {
        if (!isTransitioning(entity.getEntityWorld()) || isPrivilegedEntity(entity) || entity.isRemoved()) {
    ...
    public synchronized void prepareTransitionTick(Entity entity) {
        if (!isTransitioningOut(entity.getEntityWorld()) || isPrivilegedEntity(entity)
    ...
    public synchronized void captureResumeVelocity(Entity entity) {
        if (!isTransitioning(entity.getEntityWorld()) || !shouldTrackMomentum(entity)
    ...
    public synchronized boolean queueRedstoneInteraction(ServerPlayerEntity player, Hand hand, BlockHitResult hitResult) {
        if (!shouldFreezeWorld(player.getEntityWorld())) {
            return false;
        }
    ...
    public synchronized boolean isRestrictedPlayer(ServerPlayerEntity player) {
        return shouldFreezeWorld(player.getEntityWorld()) && !isPrivilegedEntity(player);
    }

    public synchronized boolean shouldFreezeEntity(Entity entity) {
        return shouldFreezeWorld(entity.getEntityWorld()) && !isPrivilegedEntity(entity);
    }

    public synchronized boolean shouldFreezeVolatileProgress(Entity entity) {
        return entity != null
                && isServerWorld(entity.getEntityWorld())
                && phase != StasisPhase.IDLE
    ...
    public synchronized boolean shouldAdvanceVolatileProgress(Entity entity) {
        ...
        float movementScale = StasisTimings.clamp01(getMovementMultiplier(entity.getEntityWorld()));
    ...
    public synchronized boolean shouldQueueDamage(LivingEntity entity) {
        return shouldFreezeWorld(entity.getEntityWorld()) && !isPrivilegedEntity(entity) && !applyingQueuedDamage;
    }

    public synchronized boolean shouldBypassDamageCooldown(LivingEntity entity) {
        return isTransitioningOut(entity.getEntityWorld()) && !isPrivilegedEntity(entity);
    }
    ...
    public synchronized void applyTransitionVelocity(Entity entity) {
        if (!isTransitioning(entity.getEntityWorld()) || isPrivilegedEntity(entity)) {
            return;
        }
        FrozenEntityState state = frozenEntityStates.get(entity.getUuid());
        if (state == null) {
        ...
        float targetScale = StasisTimings.clamp01(getMovementMultiplier(entity.getEntityWorld()));
    ...
    public synchronized void applyTransitionTickState(Entity entity) {
        ...
        if (state == null || !isTransitioning(entity.getEntityWorld()) || isPrivilegedEntity(entity)
                || entity.isRemoved()) {
            return;
        }
        float transitionScale = StasisTimings.clamp01(getMovementMultiplier(entity.getEntityWorld()));
        if (entity instanceof LivingEntity livingEntity) {
        ...
        if (transitionScale <= 0.0001f) {
            ...
            entity.velocityDirty = state.velocityDirty;
        } else {
            ...
            entity.velocityDirty = state.velocityDirty || entity.velocityDirty;
        }
    }
    ...
    private FrozenEntityState getOrCreateFrozenEntityState(Entity entity, boolean allowVelocityRefresh) {
        ...
        FrozenEntityState newState = new FrozenEntityState(
                ...
                getMovementMultiplier(entity.getEntityWorld())
        );
    ...
    private void replayQueuedPressureCollision(ServerWorld world, QueuedRedstoneAction action) {
        ...
        state.onEntityCollision(world, action.pos, entity, EntityCollisionHandler.DUMMY, false);
    }
    ...
    private double lerp(double from, double to, float delta) {
        return from + (to - from) * delta;
    }
    ...
    private ServerWorld getSoundWorld(MinecraftServer server) {
        ServerPlayerEntity activatingPlayer = getActivatingPlayer(server);
        if (activatingPlayer != null) {
            return activatingPlayer.getEntityWorld();
        }
    ...
    private static final class TransitionTickState {
        ...
        private TransitionTickState(
                ...
                float bodyYaw, float headYaw, double fallDistance, boolean onGround,
                boolean horizontalCollision, boolean verticalCollision,
                boolean groundCollision, boolean velocityDirty
        ) {
